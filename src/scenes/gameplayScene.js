...
export class GameplayScene {
  ...
  update(dt) {
    ...
    updateBullets(this.playerBullets, dt, this.game.height, {
      ...
      seekingTargets: this.enemies,
      boss: this.boss,
    });
    updateBullets(this.enemyBullets, dt, this.game.height, { width: this.game.width });
      ...
      for (let i = 0; i < lanes; i += 1) {
        this.enemies.push(
          this.createEnemy({
            ...
            frequency: 2.6 + i * 0.4,
            fireCooldown: 1.8,
            dropType: i === lanes - 1 ? randChoice(["speed", "canon", "spread"]) : null,
          }),
        );
      }
    } else {
      ...
      for (let i = 0; i < count; i += 1) {
        this.enemies.push(
          this.createEnemy({
            ...
            amplitude: 0,
            fireCooldown: 1.5,
            dropType: i === 0 ? "bomb" : i === count - 1 ? randChoice(["canon", "spread"]) : null,
          }),
        );
      }
    }
  ...
  spawnStageTwoWave() {
    ...
    if (pattern === "sweep") {
      ...
      for (let i = 0; i < lanes; i += 1) {
        ...
        this.enemies.push(
          this.createEnemy({
            ...
            burst: 3,
            scoreValue: 220,
            dropType: i === 1 ? randChoice(["speed", "canon", "spread", "health"]) : null,
          }),
        );
      }
    } else {
      ...
      const right = this.createEnemy({
        ...
        burstSpread: 0.35,
        health: 5,
        dropType: randChoice(["spread", "canon"]),
      });
      ...
      for (let i = 0; i < 9; i += 1) {
        this.enemies.push(
          this.createEnemy({
            ...
            fireCooldown: 1.1,
            burst: 3,
            dropType: i % 3 === 0 ? randChoice(["speed", "shield", "laser", "canon", "health"]) : null,
          }),
        );
      }
      ...
      for (let i = 0; i < 6; i += 1) {
        this.enemies.push(
          this.createEnemy({
            ...
            burst: 4,
            burstSpread: 0.4,
            dropType: i === 2 ? randChoice(["bomb", "canon", "laser"]) : null,
          }),
        );
      }
      ...
      for (let i = 0; i < count; i += 1) {
        this.enemies.push(
          this.createEnemy({
            ...
            health: 5,
            scoreValue: 260,
            dropType: i === 2 ? randChoice(["bomb", "canon", "spread"]) : null,
          }),
        );
      }
    } else {
  ...
  renderPlayerBadge(ctx, index, x, y, width, height) {
    ...
    const labelX = gaugeStartX - labelPadding;
    ctx.fillText(weaponInfo.label, labelX, height - 4);
    ctx.restore();
